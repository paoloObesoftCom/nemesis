...
class main:
	...
	def __init__(self,confdir,dbroot):
		...
		for i in temp:
			self.CHANNELLIST[i[0]] = unicode(i[1], self.REMOTE_EPG_CHARSET)
			...
			for day in self.DAYCACHE:
				...
				while i > 0  :
					...
					try:
						...
						fd = codecs.open(eventfilepath, "w", self.REMOTE_EPG_CHARSET)
				...
				for c in channels_name:
					...
					for s in sidbyname:
						...
						for e in events:
							...
							e_title = e.split(self.FIELD_SEPARATOR)[2].encode(self.REMOTE_EPG_CHARSET)

							# RAI website HAVE NOT long description. (bleah !).
							e_summarie = u' '
							# encode Python Unicode in UTF-8
							e_summarie = e_summarie.encode(self.REMOTE_EPG_CHARSET)
