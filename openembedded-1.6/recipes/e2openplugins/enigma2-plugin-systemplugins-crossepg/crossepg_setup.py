...
from Nemesis.nemesisTool import GetEpgPath
...
class CrossEPG_Setup(Screen):
	def __init__(self, session):
		...
		for partition in GetEpgPath():
			self.mountpoint.append(partition[1] + "/crossepg")
			self.mountdescription.append(partition[0])
				
		self.mountdescription.append(_("Internal flash (unsuggested)") + " (%s/data)" % self.config.home_directory)
