...
# 29-Dec-2011
# UPDATE AND MOD by Diamondear 13/01/2014
...
# sgmllib can simple parse xml data
		...
		if self.SGML_GIORNOMP != None:
			...
			for name,value in attr:
				if name == "id":
					...
					self.SGML_FD = codecs.open(eventfilepath,"w", self.REMOTE_EPG_CHARSET)
	...
	def handle_data(self, data):
		if self.SGML_TITOLO_INSIDE == True:
			self.SGML_EVENT_TITLE = data.encode(self.REMOTE_EPG_CHARSET)
			self.SGML_EVENT_TITLE = self.SGML_EVENT_TITLE.strip(' \n\r')

		if self.SGML_LINKSCHEDA_INSIDE == True:
			self.SGML_EVENT_SUMMARIE_LINK = data.encode(self.REMOTE_EPG_CHARSET)
	...
	def process_cache(self):
		...
		for f in filelist :
				...
				for c in channels_name:
					...
					for s in sidbyname:
						...
						for e in events:
							...
							e_title = items[2].encode(self.REMOTE_EPG_CHARSET)

							# extract summarie and encode Python Unicode with UTF-8
							e_summarie = items[3].encode(self.REMOTE_EPG_CHARSET)
			...
			if id == previous_id:
				...
				fd = codecs.open(os.path.join(self.CONF_CACHEDIR, f),"r", self.REMOTE_EPG_CHARSET)
				lines = fd.readlines()
				fd.close()
				if channels_name == '':
