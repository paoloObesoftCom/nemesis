...
//	}
	...
	loadedFlag |= loadSkin( formConfigPath( ICON_SKIN_NAME ), ICON_ESKIN_LOADED_VALUE_NAME, false );
	...
	sBarP->setFont( getNamedFontAndSize( EXTINFO_FONT, DEFAULT_CONF_STATUS_BAR_FONT_SIZE));
...
//	timeLabelFont.pointSize = TIME_LABEL_FONT_SIZE;
	...
	inputMgrP->checkMHW( getPlayingTSID(), inputs );
}
...
void MV::INT_MHWStatusChange( bool newStatus )
{
	setIndicator( indMHW, newStatus );
	// This works but...
	// Is reloading after MHW worthwhile ?
	// if ( newStatus == false )
...
void MV::load( time_t sampStartTime, time_t viewWindowOffset  )
{
	...
	CONNECT( inputMgrP->MHWStatusChange, MV::INT_MHWStatusChange );
...
void MV::handleCacheFill( int mode )
{
	...
	if ( fillingCacheFlag ) {
		...
		if ( 
			...
			inputMgrP->checkMHWNotRefreshing( false );
...
{
	inputMgrP->checkMHWNotRefreshing( true );
...
void MV::handleSelect( void )
{
	if ( currentChannel == -1 ) {
		...
		if ( channels[currentChannel]->isPlaying() ) {
			...
			// Check with user if we're in the middle
			// of an mhwepg scan
			inputMgrP->checkMHWNotRefreshing( true );
...
void MV::setupDataDirs( void )
{
	...
	if ( inputs.storageDir == -1 ) {
		...
		// disk whirring) and storage in /hdd for preference

		inputs.detailsDir = 6;

		if ( dirIsMountPoint( "/hdd" ) )
			inputs.storageDir = 4;
		else
			inputs.storageDir = 5;
	}
...
void MV::fillCache( bool startupFlag )
{
	...
	if ( fillingCacheFlag ) {
		...
		// If we're in the middle of an MHW refresh, wait for it to
		// finished before changing channel
		...
		// If we're scanning a particular channel, we always fill MHW if possible

		inputMgrP->checkMHW( getPlayingTSID(), inputs, ( fillCacheType == fillCacheSpecifiedChannel ) );
