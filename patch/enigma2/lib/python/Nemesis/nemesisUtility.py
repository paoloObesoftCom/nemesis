...
class NModule(Screen):
	...
	def __init__(self, session):
		...
		self.modules = [
			...
			('rt3070sta','WLAN Usb Adapter RT3070',fileExists('/lib/modules/%s/drivers/net/wireless/rt3070sta.ko' % KERNELVER)),
...
class NServicesSetup(Screen, ConfigListScreen):
	...
	def __init__(self, session):
		Screen.__init__(self, session)
		self.list = []
		self.initPath = ['/etc/rc2.d','/etc/rc3.d','/etc/rc4.d','/etc/rc5.d']
		self.servicesList = [
