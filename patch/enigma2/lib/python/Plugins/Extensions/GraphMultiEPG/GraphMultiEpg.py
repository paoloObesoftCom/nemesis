...
class EPGList(HTMLComponent, GUIComponent):
	def __init__(self, selChangedCB=None, timer = None, time_epoch = 120, overjump_empty=True):
		...
		self.backColorSelected = 0x586d88
		self.nowBackColor = 0x008d68
		self.nowBackColorSelected = 0x30d0dda8
...
#			piconbkcolor = self.backColorSelected
			piconbkcolor = 0x31000000
		else:
#			piconbkcolor = self.backColor
			piconbkcolor = 0x31000000
		...
		if events:
			...
			for ev in events:  #(event_id, event_title, begin_time, duration)
				...
				if ev[2] <= now and (ev[2] + ev[3]) > now:
					...
					backColorSelected = self.nowBackColorSelected
				else:
		...
		if cur_service:
			...
			if dir == 0: #current
				...
				if valid_event and self.cur_event-1 >= 0:
					...
					self.fillMultiEPG(None) # refill
					return True
		self.l.setSelectionClip(eRect(self.service_rect.left(), self.service_rect.top(), self.service_rect.width(), self.service_rect.height()), False)
