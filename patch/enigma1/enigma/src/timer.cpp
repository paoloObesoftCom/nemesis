...
//				conn = CONNECT( eDVB::getInstance()->leaveService, eTimerManager::leaveService );
		...
		case startEvent:
			...
			else if (nextStartingEvent->type & ePlaylistEntry::epgDown)
			{
				eDebug("[eTimerManager] Download Epg Event-type... downloading");
				eEpgManager::getInstance()->updateEpgEvent();
				writeToLogfile("Download Epg TimerEvent... download");
			}
	   ...
	   // when no ErrorCode is set the we set the state to finished
				if (nextStartingEvent->type & ePlaylistEntry::epgDown)
				{
					eDebug("[eTimerManager] Download Epg Event-type... Finished");
					eEpgManager::getInstance()->FinishedEpgEvent();
					writeToLogfile("Download Epg TimerEvent... Finished");
				}
...
#endif
	new eListBoxEntryText( *type, _("EPG Download"), (void*) ePlaylistEntry::epgDown );
...
//nemesis
	type->setCurrent( (void*) ( ttype & (ePlaylistEntry::RecTimerEntry|ePlaylistEntry::SwitchTimerEntry|ePlaylistEntry::recDVR|ePlaylistEntry::recVCR|ePlaylistEntry::recNgrab|ePlaylistEntry::epgDown ) ) );
//nemesis
	tmpService = ref;
