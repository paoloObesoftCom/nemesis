...
eString getTopNavi()
{
	...
	result += button(100, "EMUMANAGER", TOPNAVICOLOR, pre + "?mode=emu" + post);
...
eString readSrvName(eString srv)
{
	char buf[256];
	eString srvName;
...
static eString start_emu(eString request, eString dirpath, eString opts, eHTTPConnection *content)
{
	std::map<eString, eString> opt = getRequestOptions(opts, '&');
	eString emuToStart = opt["emuToStart"];
	eString command;
	eString result;

	eString emuActive = readEmuActive();
	if (emuToStart == "None" && emuActive == "None")
	{	command = _("Starting Emulator");
		result.sprintf(_("Nothing to do!")) ;
		return "<html>" + eString(CHARSETMETA) + "<head><title>" + command + "</title></head><body>" + result + "</body></html>";
	}
	eString newEmu = emuToStart;
	...
	{	nemTool::getInstance()->sendCmd((char *) eString("/var/script/" + emuActive + "_em.sh stop").c_str());
		command = _("Stopping Emulator");
		result.sprintf(_("Emulator is now stopped!"),readEmuName(newEmu).c_str()) ;
	}
	if (newEmu != "None")
	{	nemTool::getInstance()->sendCmd((char *) eString("/var/script/" + newEmu + "_em.sh start").c_str());
		command = _("Starting Emulator");
		result.sprintf(_("Emulator %s is started!"),readEmuName(newEmu).c_str()) ;
	}
	if ( isCrypted )
...
static eString start_srv(eString request, eString dirpath, eString opts, eHTTPConnection *content) 
{
	std::map<eString, eString> opt = getRequestOptions(opts, '&');
	eString srvToStart = opt["csToStart"];
	eString command;
	eString result;

	eString srvActive=readSrvActive();
	if (srvToStart == "None" && srvActive == "None")
	{	command = _("Starting Cardserver");
		result.sprintf(_("Nothing to do!")) ;
		return "<html>" + eString(CHARSETMETA) + "<head><title>" + command + "</title></head><body>" + result + "</body></html>";
	}
	eString emuActive = readEmuActive();
	eString emuName = readEmuName(emuActive);
	eString newSrv = srvToStart;
	if (emuActive != "None" && srvActive == "None")
 	{	command = _("Error!");
		result.sprintf(_("The emulator '%s' is started!<br>Please stop it before start cardserver."),emuName.c_str());
		return "<html>" + eString(CHARSETMETA) + "<head><title>" + command + "</title></head><body>" + result + "</body></html>";
	}
	...
	{	nemTool::getInstance()->sendCmd((char *) eString("/var/script/" + srvActive + "_cs.sh stop").c_str());
		command = _("Stopping Cardserver");
		result.sprintf(_("Cardserver is now stopped!"),readSrvName(newSrv).c_str()) ;
	}
	if (newSrv != "None")
	{	nemTool::getInstance()->sendCmd((char *) eString("/var/script/" + newSrv + "_cs.sh start").c_str());
		command = _("Starting Cardserver");
		result.sprintf(_("Cardserver %s is started!"),readSrvName(newSrv).c_str()) ;
	}
	return "<html>" + eString(CHARSETMETA) + "<head><title>" + command + "</title></head><body>" + result + "</body></html>";
}
...
eString getContent(eString mode, eString path, eString opts)
{
	...
	if (mode == "emu")
	{
		result = getTitle("EDG-Nemesis Emu Manager");
		result += readFile(TEMPLATE_DIR + "edg-nemesis.tmp");
		...
		//read emu list
			...
			eString emuList = "";
			eString isSelect = "";
			if (emuActive == "None") isSelect = "selected";
			if ((eSystemInfo::getInstance()->getHwType() ==  eSystemInfo::DM500) || 
				(eSystemInfo::getInstance()->getHwType() ==  eSystemInfo::DM500PLUS))
				emuList = "<option value=None " + isSelect + ">None</option>";
			else
				emuList = "<option value=None " + isSelect + ">Common Interface</option>";
			char emuname[100];
			...
			{	while (struct dirent *e=readdir(d))
				{	if ( strstr(e->d_name,"_em.sh")) 
					{	memset(emuname, 0, sizeof(emuname));
						...
						if (eString(emuname) == emuActive) 
							isSelect = "selected";
						else
							isSelect = "";
						emuList +="<option value=" +  eString(emuname) + " " + isSelect + ">" + readEmuName(emuname) + "</option>";
					}
				}
				closedir(d);
			}
			result.strReplace("#EMU#", emuList);
		//end read emu list
		...
		//read cs list
			...
			if (srvActive == "None") 
				...
				isSelect = "";
			eString srvList = "<option value=None " + isSelect + ">None</option>";
			char srvname[100];
			DIR *s = opendir("/var/script");
			if (s)
			{	while (struct dirent *e=readdir(s))
				{	if ( strstr(e->d_name,"_cs.sh")) 
					{	memset(srvname, 0, sizeof(srvname));
						strncpy(srvname, e->d_name, strlen(e->d_name)-6);
						if (eString(srvname) == srvActive) 
							isSelect = "selected";
						else
							isSelect = "";
						srvList +="<option value=" +  eString(srvname) + " " + isSelect + ">" + readSrvName(srvname) + "</option>";
					}
				}
				closedir(s);
			}
			result.strReplace("#CS#", srvList);
		//end read cs list

		//read ecm.info
			eString ecmTextInfo = "";
			char ibuf[256];
			if( eServiceInterface::getInstance()->getService()->getFlags() & eServiceHandler::flagIsScrambled )
			{	FILE *f = fopen("/tmp/ecm.info", "r");
				...
				{	int len = 0;
					...
					{	len += strlen(ibuf);
						if (len <= 65536)
							ecmTextInfo += eString().sprintf("%s <br>", ibuf);
					}
					fclose(f);
				}
			}else 
				ecmTextInfo = _("No ECM info");
			result.strReplace("#EMUINFO#", ecmTextInfo );
		//end read ecm.info
...
InputDevFound:
		if (evd > -1)
		{
			eDebug("Keypress:%d",key);
			sendKey(evd, key, KEY_PRESSED);
			while (time--)
			{
				usleep(reptime * 1000);
				eDebug("Keyrepeat:%d",key);
				sendKey(evd, key, KEY_AUTOREPEAT);
			}
			eDebug("Keyreleased:%d",key);
...
//nemesis	
	dyn_resolver->addDyn("GET", "/NT/startEmu", start_emu, lockWeb);
	dyn_resolver->addDyn("GET", "/NT/startCs", start_srv, lockWeb);
