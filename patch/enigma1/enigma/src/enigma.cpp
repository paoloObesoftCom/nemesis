...
eString eZap::readEmuName(eString emu)
{
	char buf[256];
	eString emuName;
...
eString eZap::readSrvName(eString srv)
{
	char buf[256];
	eString srvName;
...
void eZap::stopEmu()
{
	...
	if (emuAct != "None")
	{
		eDebug("[ENIGMA] stop emulator");
		eMessageBox box1(eString().sprintf(_("Stopping %s"), readEmuName(emuAct).c_str()), _("Running.."), 0);
		box1.move(ePoint(250, 300));
		box1.resize(eSize(400, 120));
		box1.show();
			nemTool::getInstance()->sendCmd((char *) eString("/var/script/" + emuAct + "_em.sh stop").c_str());
		box1.hide();
	}
}
...
void eZap::startEmu()
{
	...
	if (emuAct != "None")
	{
		eDebug("[ENIGMA] start emulator");
		int emucsstartdisable=0;
		eConfig::getInstance()->getKey("/Nemesis-Project/tools/emucsstartdisable", emucsstartdisable);
		if (!emucsstartdisable)
		{	eMessageBox box(eString().sprintf(_("Starting %s"), readEmuName(emuAct).c_str()), _("Running.."), 0);
			box.move(ePoint(250, 300));
			box.resize(eSize(400, 120));
			box.show();
				nemTool::getInstance()->sendCmd((char *) eString("/var/script/" + emuAct + "_em.sh start").c_str());
		box.hide();	
		}else	
			unlink("/var/bin/emuactive");
 	}
}
...
void eZap::stopCs()
{
	...
	{	fgets(buf, 256, in);
 	...
 	if (srvAct != "None")
 	{
		eDebug("[ENIGMA] stop cardserver");
		eMessageBox box1(eString().sprintf(_("Stopping %s"), readSrvName(srvAct).c_str()), _("Running.."), 0);
		box1.move(ePoint(250, 300));
		box1.resize(eSize(400, 120));
		box1.show();
 			nemTool::getInstance()->sendCmd((char *) eString("/var/script/" + srvAct + "_cs.sh stop").c_str());
		box1.hide();
 	}
}
...
void eZap::startCs()
{
	...
	{	fgets(buf, 256, in);
 	...
 	if (srvAct != "None")
 	{
		eDebug("[ENIGMA] start cardserver");
		int emucsstartdisable=0;
		eConfig::getInstance()->getKey("/Nemesis-Project/tools/emucsstartdisable", emucsstartdisable);
		if (!emucsstartdisable)
		{	eMessageBox box(eString().sprintf(_("Starting %s"), readSrvName(srvAct).c_str()), _("Running.."), 0);
			box.move(ePoint(250, 300));
			box.resize(eSize(400, 120));
			box.show();
 				nemTool::getInstance()->sendCmd((char *) eString("/var/script/" + srvAct + "_cs.sh start").c_str());
			box.hide();	
		}else	
			unlink("/var/bin/csactive");
 	}
}
