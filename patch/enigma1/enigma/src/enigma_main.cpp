...
//nemesis
		green_repeat, green_release,
		yellow_repeat, yellow_release,
		blue_press, blue_repeat, blue_release,
//
	...
	enigmaMainActions():
		...
		green_repeat(map, "green_repeat", _("open EPG control center Panel (hold)"), eAction::prioDialog),
		green_release(map, "green_release", _("show subservices/NVOD (release)"), eAction::prioDialog),
		yellow_repeat(map, "yellow_repeat", _("open EMU/CS Information (hold)"), eAction::prioDialog),
		yellow_release(map, "yellow_release", _("open plugin panel (release)"), eAction::prioDialog),
		blue_press(map, "blue_press", _("open Nemesis Panel"), eAction::prioDialog),
		blue_repeat(map, "blue_repeat", _("open ECM Information (hold)"), eAction::prioDialog),
		blue_release(map, "blue_release", _("open Nemesis Panel (release)"), eAction::prioDialog),
...
void eZapMain::cleanPlaylist(bool destroy)
{
	// clean playlist
	playlist->clear();
	if (destroy)
		unlink(eString().sprintf("%s/playlist.epl",eplPath.c_str()).c_str());
}
...
void eZapMain::loadRecordings( bool create )
{
	...
	if (!eConfig::getInstance()->getKey("/Nemesis-Project/PVR/moviedir", eMoviePath))
		recordings->load(eString().sprintf("%s/recordings.epl",eMoviePath).c_str());
	else
...
void eZapMain::init_main()
{
	...
	int autostandby;
	if ((eConfig::getInstance()->getKey("/extras/autostandby", autostandby)) != 0)
	{
		autostandby=0;
		eConfig::getInstance()->setKey("/extras/autostandby", autostandby);
	}
	
	int wasShutdown=0;
	eConfig::getInstance()->getKey("/extras/wasshutdown", wasShutdown );
	
	if (autostandby!=0 && wasDeepstandby==0 && wasShutdown==0)
	{  
		if (autostandby==3)
			if (handleStandby(3))
				eDebug ("Is it stormy?");
		if (autostandby==2)
		{
			eMessageBox mb(_("Shutdown your Receiver now?"),_("Timer Message"), eMessageBox::btYes|eMessageBox::btNo|eMessageBox::iconQuestion, eMessageBox::btYes, 10);
			mb.show();
			int ret = mb.exec();
			mb.hide();
	...
	eConfig::getInstance()->setKey("/extras/wasshutdown", 0 );
	eConfig::getInstance()->flush();
...
// Volume Slider
	gPixmap *spm = eSkin::getActive()->queryImage("volume_slider");

	if (spm)
		VolumeBar->setSliderPixmap(spm);
	else
	{
...
#endif
// clock Patch
	ASSIGN(hour_gauge, eGauge, "hour_gauge");
	ASSIGN(minutes_gauge, eGauge, "minutes_gauge");
	ASSIGN(seconds_gauge, eGauge, "seconds_gauge");
// SNR Patch
...
// SNR Patch
	...
	ASSIGN(nds_emm, eLabel ,"nds_emm");
	ASSIGN(via_emm, eLabel ,"via_emm");
	ASSIGN(cw_emm, eLabel ,"cw_emm");
	ASSIGN(beta_emm, eLabel ,"beta_emm");
	ASSIGN(conax_emm, eLabel ,"conax_emm");
	ASSIGN(seca_emm, eLabel ,"seca_emm");
	ASSIGN(irdeto_emm, eLabel ,"irdeto_emm");
	ASSIGN(nagra_emm, eLabel ,"nagra_emm");
	ASSIGN(dc_emm, eLabel ,"dc_emm");
	
	ASSIGN(nds_ecm, eLabel ,"nds_ecm");
	ASSIGN(via_ecm, eLabel ,"via_ecm");
	ASSIGN(cw_ecm, eLabel ,"cw_ecm");
	ASSIGN(beta_ecm, eLabel ,"beta_ecm");
	ASSIGN(conax_ecm, eLabel ,"conax_ecm");
	ASSIGN(seca_ecm, eLabel ,"seca_ecm");
	ASSIGN(irdeto_ecm, eLabel ,"irdeto_ecm");
	ASSIGN(nagra_ecm, eLabel ,"nagra_ecm");
	ASSIGN(dc_ecm, eLabel ,"dc_ecm");

	ASSIGN(nds_no, eLabel ,"nds_no");
	ASSIGN(via_no, eLabel ,"via_no");
	ASSIGN(cw_no, eLabel ,"cw_no");
	ASSIGN(beta_no, eLabel ,"beta_no");
	ASSIGN(conax_no, eLabel ,"conax_no");
	ASSIGN(seca_no, eLabel ,"seca_no");
	ASSIGN(irdeto_no, eLabel ,"irdeto_no");
	ASSIGN(nagra_no, eLabel ,"nagra_no");
	ASSIGN(dc_no, eLabel ,"dc_no");

	dc_emm->hide();nds_emm->hide();via_emm->hide();cw_emm->hide();beta_emm->hide();conax_emm->hide();seca_emm->hide();irdeto_emm->hide();nagra_emm->hide();
	dc_ecm->hide();nds_ecm->hide();via_ecm->hide();cw_ecm->hide();beta_ecm->hide();conax_ecm->hide();seca_ecm->hide();irdeto_ecm->hide();nagra_ecm->hide();
	dc_no->show();nds_no->show();via_no->show();cw_no->show();beta_no->show();conax_no->show();seca_no->show();irdeto_no->show();nagra_no->show();

	ASSIGN(picon, eLabel ,"picon");picon->hide();
	ASSIGN(picon_dvr, eLabel ,"picon_dvr");picon_dvr->hide();
	ASSIGN(cam_info, eLabel ,"cam_info");cam_info->show();
 	ASSIGN(ecm_info, eLabel ,"ecm_info");ecm_info->show();
	...
	ASSIGN(fc_fec, eLabel ,"fc_fec");fc_fec->show();
 	ASSIGN(pro_name, eLabel ,"pro_name");pro_name->show();
	...
	ASSIGN(netcard_info, eLabel ,"netcard_info");
	ASSIGN(videoformat, eLabel ,"videoformat");
	ASSIGN(button_card, eLabel ,"button_card");
	ASSIGN(button_emu, eLabel ,"button_emu");
	ASSIGN(button_spider, eLabel ,"button_spider");
	button_spider->hide();button_card->hide();button_emu->hide();netcard_info->show();videoformat->show();
...
void eZapMain::getCodInfo()
{
	eDebug("Get video format");
	videoformat->setText(osdInfo::getInstance()->getVideoInfo());

	int disableEcm = 0;
	eConfig::getInstance()->getKey("/Nemesis-Project/infobar/disableecm", disableEcm);
	if (!disableEcm)
	{
		if(isCrypted==1)
		{	
			...
			int ihops = 0;
			int *hop = &ihops;
			osdInfo::getInstance()->getData(caid,pid,provid,ptr1,addr,port,ecmt,hop,reader);
	
			if (strncmp(caid,"",1) != 0)
				ca_id->setText(eString().sprintf("Ca ID: %s", caid));
			if (strncmp(provid,"",1) != 0)
				prov_id->setText(eString().sprintf("Prov ID: %s", provid));
			if (strncmp(pid,"",1) != 0)
				pid_id->setText(eString().sprintf("Pid: %s", pid));
			if (ecmtime != 0)
				ecm_time->setText(eString().sprintf("ECM Time: %d msec", ecmtime));

			int disableNet = 0;
			eConfig::getInstance()->getKey("/Nemesis-Project/infobar/disablenet", disableNet);

			switch (source)
			{
				case 1:
					button_emu->show(); netcard_info->setText("Decode: Internal"); break;
				case 2: 
					if (strncmp(addr,"None",4) == 0)
					{	button_spider->show(); 
						netcard_info->setText("Decode: Network"); 
					}else
					{	
						...
						{	
							if (strncmp(port,"",1) == 0)
							{	if (strncmp(reader,"",1) != 0)
									netcard_info->setText(eString().sprintf("Source: %s Reader: %s", addr, reader)); 
								else	
									netcard_info->setText(eString().sprintf("Source: %s", addr)); 
							}else
							{	if (strncmp(reader,"",1) != 0)
									netcard_info->setText(eString().sprintf("Source: %s:%s Reader: %s", addr, port, reader));
								else
									netcard_info->setText(eString().sprintf("Source: %s:%s", addr, port)); 
							}
							hops->setText(eString().sprintf("Hops: %d", ihops));
						}else
						{	if (strncmp(addr,"127.0.0.1",9) == 0 || strncmp(addr,"localhost",9) == 0)
								netcard_info->setText("Decode: Smartcard"); 
							else
								netcard_info->setText("Decode: Network"); 
						}
					}
				...
				case 3: 
					...
					if (strncmp(reader,"",1) == 0)
						...
						netcard_info->setText(eString().sprintf("Decode: %s", reader));
					break;
				case 4: button_card->show(); netcard_info->setText("Decode: slot-1"); break;
				case 5: button_card->show(); netcard_info->setText("Decode: slot-2"); break;
			}
	
			char *cod = ""; eString ePar;
			ePar = eString().sprintf("%s", caid);
			if (strstr(ePar.c_str(),"0x01")) cod = "S";
			if (strstr(ePar.c_str(),"0x1"))  cod = "S";
			if (strstr(ePar.c_str(),"0x05")) cod = "V";
			if (strstr(ePar.c_str(),"0x5"))  cod = "V";
			if (strstr(ePar.c_str(),"0x06")) cod = "I";
			if (strstr(ePar.c_str(),"0x6"))  cod = "I";
			if (strstr(ePar.c_str(),"0x09")) cod = "NDS";
			if (strstr(ePar.c_str(),"0x9"))  cod = "NDS";
			if (strstr(ePar.c_str(),"0x0b")) cod = "CO";
			if (strstr(ePar.c_str(),"0x0B")) cod = "CO";
			if (strstr(ePar.c_str(),"0xb"))  cod = "CO";
			if (strstr(ePar.c_str(),"0xB"))  cod = "CO";
			if (strstr(ePar.c_str(),"0x0d")) cod = "CW";
			if (strstr(ePar.c_str(),"0x0D")) cod = "CW";
			if (strstr(ePar.c_str(),"0xd"))  cod = "CW";
			if (strstr(ePar.c_str(),"0xD"))  cod = "CW";
			if (strstr(ePar.c_str(),"0x17")) cod = "B";
			if (strstr(ePar.c_str(),"0x18")) cod = "N";
			if (strstr(ePar.c_str(),"0x4a7")) cod = "DC";
			if (strstr(ePar.c_str(),"0x4A7")) cod = "DC";
	
			if ((cod == "S") && seca)
			{	seca_emm->hide(); seca_ecm->show();
			}
			if ((cod == "N") && nagra)
			{	nagra_emm->hide(); nagra_ecm->show();
			}
			if ((cod == "V") && via)
			{	via_emm->hide(); via_ecm->show();
			}
			if ((cod == "I" || cod == "DC") && irdeto)
			{	irdeto_emm->hide(); irdeto_ecm->show();
			}
			if ((cod == "NDS") && nds)
			{	nds_emm->hide(); nds_ecm->show();
			} 
			if ((cod == "B") && beta)
			{	beta_emm->hide(); beta_ecm->show();
			}
			if ((cod == "CO") && conax)
			{	conax_emm->hide(); conax_ecm->show();
			}
			if ((cod == "CW") && cw)
			{	cw_emm->hide(); cw_ecm->show();
			}
			if ((cod == "DC") && dc)
			{	int showDcAsIrdeto = 0;
				eConfig::getInstance()->getKey("/Nemesis-Project/infobar/showdcasirdeto", showDcAsIrdeto);
				if (showDcAsIrdeto)
				{	irdeto_emm->hide(); 
					irdeto_ecm->show();
				}
			...
			netcard_info->setText("Free To Air"); 
	}
}

void eZapMain::clearInfo()
{
	ecm_info->show();
	button_spider->hide();button_card->hide();button_emu->hide();
	netcard_info->setText("");videoformat->setText("");pro_name->setText("");
	prov_id->setText("");pid_id->setText("");ca_id->setText("");ecm_time->setText("");hops->setText("");
	dc =false;nds= false,via= false,seca= false,irdeto= false,beta= false,cw= false,conax= false,nagra = false;
	dc_emm->hide();nds_emm->hide();via_emm->hide();cw_emm->hide();beta_emm->hide();conax_emm->hide();seca_emm->hide();irdeto_emm->hide();nagra_emm->hide();
	dc_ecm->hide();nds_ecm->hide();via_ecm->hide();cw_ecm->hide();beta_ecm->hide();conax_ecm->hide();seca_ecm->hide();irdeto_ecm->hide();nagra_ecm->hide();
	dc_no->show();nds_no->show();via_no->show();cw_no->show();beta_no->show();conax_no->show();seca_no->show();irdeto_no->show();nagra_no->show();
}

void eZapMain::hiddenLCD( int stateLCD )
{
	int lcd_hid=0;
	...
	if(!(state & stateSleeping))
	{
		...
		case 1:
					...
					{
					if ( lcd_hid == 1 )
						eDBoxLCD::getInstance()->switchLCD(2);
					else
						eDBoxLCD::getInstance()->switchLCD(1);
					}
			break;
...
void eZapMain::getPicon()
{
		...
		{	eService *service = eServiceInterface::getInstance()->addRef(ref);
			...
			{	if (ref.type == eServiceReference::idDVB)
				{	
					...
					{	name=eString(ref.toString()).upper();
						name=nemTool::getInstance()->replace(name,":","_");
						name=name.left(name.length() - 1);
					}
					if (name.length())
						piconFile = eString().sprintf("%spicon/%s.png",piconPath,name.c_str());
				}
	...
	eDebug("Picon Name: %s",piconFile.c_str());
	if (!nemTool::getInstance()->exists((char*) piconFile.c_str() ,false))
		piconFile = "/var/tuxbox/config/default.png";
	gPixmap *img = loadPNG(piconFile.c_str());
	if (img)
	{	
...
void eZapMain::getScreenInfo()
{
	char buffer [80];
	int showLongDate = 0;
	eConfig::getInstance()->getKey("/Nemesis-Project/infobar/showlongdate", showLongDate);
	if (showLongDate)
		osdInfo::getInstance()->getFormattedData(buffer);
	else
		osdInfo::getInstance()->getFormattedDataShort(buffer);
	DDate->setText(eString().sprintf("%s",buffer));

	eDebug("Data=%s",buffer);

	eDVBServiceController *sapi = eDVB::getInstance()->getServiceAPI();
	if(sapi)
	{
		int freqency = 0; int *freq = &freqency; 
		int simbolrate = 0; int *sr = &simbolrate;
		char  pol[2] = ""; char fec[6] = ""; char proname[256] = "";

		osdInfo::getInstance()->getLNBData(freq,sr,pol,fec,proname);

		eDebug("feq=%i %s",freqency, pol);
		eDebug("snr=%i",simbolrate);
		eDebug("fec=%i",fec);

		feq->setText(eString().sprintf("%i %s", freqency, pol).c_str());
		sr_snr->setText(eString().sprintf("%i",simbolrate).c_str());
		fc_fec->setText(eString().sprintf("%s",fec).c_str());
		
		int showPrvWithCahnnel = 0;
		eConfig::getInstance()->getKey("/Nemesis-Project/infobar/showprowithchan", showPrvWithCahnnel);
		if (!showPrvWithCahnnel)
			pro_name->setText(eString().sprintf("%s",proname).c_str());

		eDebug("proname=%s",proname);
		...
		if(isCrypted==1)
		{
			...
			for (std::set<int>::iterator i(calist.begin()); i != calist.end(); ++i)
			{
				...
				if(strstr(caname.c_str(),"Viacces"))
				{
					via_no->hide();
					via_emm->show();
					via=true;
				}
				if(strstr(caname.c_str(),"Videoguard"))
				{
					nds_no->hide();
					nds_emm->show();
					nds=true;
				}
				if(strstr(caname.c_str(),"Seca"))
				{
					seca_no->hide();
					seca_emm->show();
					seca=true;
				}
				if(strstr(caname.c_str(),"Irdeto"))
				{
					irdeto_no->hide();
					irdeto_emm->show();
					irdeto=true;
				}
				if(strstr(caname.c_str(),"Cryptoworks"))
				{
					cw_no->hide();
					cw_emm->show();
					cw=true;
				}
				if(strstr(caname.c_str(),"Betacrypt"))
				{
					beta_no->hide();
					beta_emm->show();
					beta=true;
				}
				if(strstr(caname.c_str(),"Kudelski"))
				{
					nagra_no->hide();
					nagra_emm->show();
					nagra=true;
				}
				if(strstr(caname.c_str(),"Conax"))
				{
					conax_no->hide();
					conax_emm->show();
					conax=true;
				}
				if(strstr(caname.c_str(),"DreamCrypt"))
				{	int showDcAsIrdeto = 0;
					eConfig::getInstance()->getKey("/Nemesis-Project/infobar/showdcasirdeto", showDcAsIrdeto);
					if (showDcAsIrdeto)
					{	irdeto_no->hide();
...
// 	addActionToHelpList(&i_enigmaMainActions->yellow_repeat);
	addActionToHelpList(&i_enigmaMainActions->blue_press);
// 	addActionToHelpList(&i_enigmaMainActions->blue_repeat);
}
...
void eZapMain::showInfobar(bool startTimeout)
{
	...
	eConfig::getInstance()->getKey("/Nemesis-Project/infobar/changed_offset", changed_offset );
	if (lightmode!=0 || changed_offset!=0)
	{	
		if (isVisible())
			hideInfobar();
		if (changed_offset!=0)
			eConfig::getInstance()->setKey("/Nemesis-Project/infobar/changed_offset", 0 );
// 		eZap::getInstance()->getDesktop(eZap::desktopFB)->invalidate( eRect( 0, 0, 720, 576 ), 1);
	...
	if (dvrfunctions)
	{
		eString piconFile = "/var/tuxbox/config/record.png";
		gPixmap *img = loadPNG(piconFile.c_str());
		if (img)
		{	gPixmap *mp = &gFBDC::getInstance()->getPixmap();
			gPixmapDC mydc(img);
			gPainter p(mydc);
			p.mergePalette(*mp);
			picon_dvr->setPixmap(img);
			picon_dvr->show();
		}else
			picon_dvr->setPixmap(0);
	}
	...
	eDebug("clearInfo");
	clearInfo();	
	eDebug("clockOsdUpdate");
	clockOsdUpdate();
	eDebug("getScreenInfo");
	getScreenInfo();
	eDebug("cam_info");
	cam_info->setText(osdInfo::getInstance()->getEmuInfo());

	if(!codTimer)
	{
		eDebug("getCodInfo");
		codTimer=new eTimer(eApp);
		codTimer->start(600,false); 
		CONNECT(codTimer->timeout,eZapMain::getCodInfo);
	}
...
void eZapMain::showLightInfobar(bool startTimeout)
{
	picon_dvr->hide();
	...
	int changed_offset = 0;
	eConfig::getInstance()->getKey("/Nemesis-Project/infobar/changed_offset", changed_offset );
	if (lightmode!=1 || changed_offset!=0)
	{	
		if (isVisible())
			hideInfobar();
		if (changed_offset!=0)
...
//			eDebug("filename = descr = %s", descr.c_str() );
		...
		do
		{
			char *eMoviePath = 0;
			if (!eConfig::getInstance()->getKey("/Nemesis-Project/PVR/moviedir", eMoviePath))
				filename=eString().sprintf("%s/",eMoviePath).c_str();
			else
				filename=MOVIEDIR "/";
...
void eZapMain::startSkip(int dir)
{
	...
	{ 
		...
		if (handler)
		{
			...
			seekstart=handler->getPosition(eServiceHandler::posQueryCurrent);
			seekpos=seekstart;
		}
	}
...
void eZapMain::renameFile( eServiceSelector *sel )
{
	...
	TextEditWindow wnd(_("Enter new Filename:"),"abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890 -_öüäÖÜÄ");
...
void eZapMain::greenPress()
{
	gettimeofday(&greenTime,0);
	greenTime+=1000;
}

void eZapMain::greenRepeat()
{
	timeval now;
	gettimeofday(&now,0);
	if ( greenTime < now )
...
void eZapMain::greenRelease()
{
	if ( greenTime.tv_sec == -1)
		return;

	timeval now;
	gettimeofday(&now,0);
	bool b = greenTime < now;
	if (b)
	{
		greenTime.tv_sec=-1;
#ifndef DISABLE_FILE
	...
	{
		greenTime.tv_sec=-1;
		if (!dvrInfoBar->isVisible())
			if (( (mode == modeTV) || (mode == modeRadio)) && (state < 2))
				openEpgControlCenter();
	}
}

void eZapMain::openEpgControlCenter()
{
	hide();
	eEpgManager dlg;
	dlg.show();
...
void eZapMain::runPluginMV()
{
	eZapPlugins plugins(eZapPlugins::StandardPlugin);
	hide();
	if ( plugins.execPluginByName("mv.cfg") != "OK" )
		plugins.execPluginByName("_mv.cfg");
	if (!doHideInfobar())
		showInfobar();
}

void eZapMain::runPluginEPGui()
{
	eZapPlugins plugins(eZapPlugins::StandardPlugin);
	hide();
	if ( plugins.execPluginByName("epgui.cfg") != "OK" )
		plugins.execPluginByName("_epgui.cfg");
	if (!doHideInfobar())
		showInfobar();
}

void eZapMain::runPluginWeekEPG()
{
	eZapPlugins plugins(eZapPlugins::StandardPlugin);
	hide();
	if ( plugins.execPluginByName("weekepg.cfg") != "OK" )
		plugins.execPluginByName("_weekepg.cfg");
	if (!doHideInfobar())
		showInfobar();
}

void eZapMain::runPluginEpgFind()
{
	eZapPlugins plugins(eZapPlugins::StandardPlugin);
	hide();
	if ( plugins.execPluginByName("epgfind.cfg") != "OK" )
		plugins.execPluginByName("_epgfind.cfg");
	if (!doHideInfobar())
		showInfobar();
}
...
int eZapMain::eventHandler(const eWidgetEvent &event)
{
	...
	case eWidgetEvent::evtAction:
	{
		...
		else if ( !isVisible() && event.action == &i_enigmaMainActions->showInfobar)
		{
			...
			int skinlight = 0;
			if (!dvrfunctions)
			{	eConfig::getInstance()->getKey("/Nemesis-Project/tools/liteskin", skinlight);
				if (skinlight)
					showLightInfobar(true);
				else
					showInfobar(true);
			}else
				showLightInfobar(true);
...
/*		{
		...
		else if (event.action == &i_enigmaMainActions->green_repeat)
			greenRepeat();
		else if (event.action == &i_enigmaMainActions->green_release)
			greenRelease();
		else if (event.action == &i_enigmaMainActions->showAudioOrPause)
		{
			...
			runVTXT();
		else if (event.action == &i_enigmaMainActions->blue_press)
			bluePress();
		else if (event.action == &i_enigmaMainActions->blue_repeat)
			blueRepeat();
		else if (event.action == &i_enigmaMainActions->blue_release)
			blueRelease();
		else if (event.action == &i_enigmaMainActions->pluginExt)
			yellowPress();
		else if (event.action == &i_enigmaMainActions->yellow_repeat)
			yellowRepeat();
		else if (event.action == &i_enigmaMainActions->yellow_release)
			yellowRelease();
		...
		else if (event.action == &i_enigmaMainActions->toggleDVRFunctions)
		{
			...
			int skinlight = 0;
			if (!dvrfunctions)
			{	eConfig::getInstance()->getKey("/Nemesis-Project/tools/liteskinzap", skinlight);
				if (skinlight)
					showLightInfobar(true);
				else
					showInfobar(true);
		...
		if (handler)
		{
			...
			if ( num && ( (myref.type == eServiceReference::idDVB && myref.path)
				...
				&& myref.data[0] == eMP3Decoder::codecOGG ) || timeshift) && (handler->getState() == eServiceHandler::statePlaying || handler->getState() == eServiceHandler::statePause)) // nur, wenn ts, mpg oder mp3 ausgew?�hlt ist und vor allem, wenn es abgespielt wird oder im Standbild ist! :-)
			{
				if (handler->getState() == eServiceHandler::statePause)
					pause();// continue playing in preparation for skipping
				int time=0;
...
//		setVTButton(isVT);
  ...
  // need this compare since the dvb service type is no more compared in eServiceReference ==, !=, <
				...
				eSystemInfo::getInstance()->getFEType() == eSystemInfo::feSatellite )
		{
			...
			eDVBServiceController *pro_sapi = eDVB::getInstance()->getServiceAPI();
			if(pro_sapi)
			{
				eServiceDVB *service=eDVB::getInstance()->settings->getTransponders()->searchService(pro_sapi->service);
				if (service)
					pro_name = (osdInfo::getInstance()->replace(filter_string(service->service_provider), "&","&amp;"));
			}
			...
			{	if (showPrvWithCahnnel)
					name+=eString().sprintf(" (%d.%d\xC2\xB0%c - %s)", abs(opos / 10), abs(opos % 10), opos>0?'E':'W', pro_name.c_str());
				else
					name+=eString().sprintf(" (%d.%d\xC2\xB0%c)", abs(opos / 10), abs(opos % 10), opos>0?'E':'W');
			}else
			{
				if (showPrvWithCahnnel && pro_sapi)
					name+=eString().sprintf(" - %s", pro_name.c_str());
			}				
		...
		int showPicon = 0;
		eConfig::getInstance()->getKey("/Nemesis-Project/tools/showpicon", showPicon);
		if (showPicon)
			getPicon();
...
void eZapMain::clockOsdUpdate()
{
	...
	{
		eString s;
		s.sprintf("%02d:%02d:%02d", t->tm_hour, t->tm_min, t->tm_sec);
		clocktimer.start(1001);
		Clock->setText(s);
		
		eDebug("Clock: %02d:%02d:%02d", t->tm_hour, t->tm_min, t->tm_sec);

		int g_h;
		if (t->tm_hour > 12)
			g_h = ((t->tm_hour - 12) * 60 + t->tm_min) * 100 / 720;
		else
			g_h = (t->tm_hour * 60 + t->tm_min) * 100 / 720;
		int g_m = t->tm_min * 100 / 60;
		int g_s = t->tm_sec * 100 / 60;

		hour_gauge->setPerc(g_h);
		minutes_gauge->setPerc(g_m);
		seconds_gauge->setPerc(g_s);
	}else
	{
		Clock->setText("--:--:--");
		hour_gauge->setPerc(0);
		minutes_gauge->setPerc(0);
		seconds_gauge->setPerc(0);
	}
}
