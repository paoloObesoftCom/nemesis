...
private:
	eLabel 	*ChannelNumber, *ChannelName, *Clock,
		...
		*button_card, *button_emu, *button_spider, *netcard_info, *videoformat, *DDate, *picon, *picon_dvr;

	eGauge *hour_gauge, *minutes_gauge, *seconds_gauge;
...
/* SNR,AGC,BER DISPLAY */
	...
	void greenRepeat();
	void greenRelease();
	void openEpgControlCenter();

	void hiddenLCD(int state);
	void getScreenInfo();
	void getCodInfo();
	void clearInfo();
	void getPicon();
	...
	void runPluginMV();
	void runPluginEPGui();
	void runPluginWeekEPG();
	void runPluginEpgFind();
	...
	void showLightInfobar(bool startTimeout=false);
// 	void showFileInfobar(bool startTimeout=false);
...
public:
	...
	//Nemesis
	void cleanPlaylist(bool destroy = false);
