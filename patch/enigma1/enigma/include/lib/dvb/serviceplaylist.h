...
//  Repeated Timer Days
	...
	{ }
	ePlaylistEntry(const eServiceReference &ref, int time_begin, int type=epgDown )
		:services(ref), service(services.path.back()), event_id(event_id), time_begin(time_begin), type(type)
	{ }
