...
{
	FILE *fd = NULL;
	fd = fopen ("/etc/enigma/epg.blacklist", "r");
	if (fd)
	{
		char line[256];
...
bool eEPGCache::CheckEPGBlock ()
{
	...
	int epgBlkEnabled = 0;
	eConfig::getInstance()->getKey("/Nemesis-Project/tools/enaepgbl", epgBlkEnabled);
	...
	if (epgBlkEnabled)
	{
		...
		if (!ref)
			return false;
		transport_stream_id = ref.getTransportStreamID().get();
		original_network_id = ref.getOriginalNetworkID().get();
